...
export class Terrain {
  ...
  initializeNoise() {
      ...
      // Since simplex-noise library uses Math.random() internally if not provided?
      // The library 'simplex-noise' v4 provides createNoise2D(randomFunc).
  ...
  getBlockId(x, y, z) {
    ...
    if (!chunk) {
        ...
        if (y > maxTerrainBlockY) return 0; // Air

        const surfaceHeight = this.getHeightAtWorld(x, z);
        if (y <= surfaceHeight) {
             // Fix: Layers relative to surface
             if (y === surfaceHeight) {
                 return y >= 25 ? BLOCK_IDS['snow'] : BLOCK_IDS['grass'];
             }
             if (y > surfaceHeight - 4) return BLOCK_IDS['dirt'];
  ...
  getHeightAtWorld(x, z) {
    ...
    const n = this.noise2D(nx, nz);
    return Math.floor(this.baseHeight + ((n + 1) * 0.5) * this.amplitude);
  }
          ...
          for (let z = 0; z < CHUNK_SIZE; z++) {
              ...
              for (let y = 0; y < CHUNK_SIZE; y++) {
                  ...
                  if (!skipProcedural) {
                      ...
                      chunk.setProceduralBlockId(x, y, z, BLOCK_IDS[type]);
                  } else {
                      chunk.setProceduralBlockId(x, y, z, BLOCK_IDS['air']);
                  }
              }
          }
      }
