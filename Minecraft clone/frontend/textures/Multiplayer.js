...
export class Multiplayer {
    ...
    connect() {
        ...
        // Automatically detect if we are on localhost or production (Render/etc.)
        // If on localhost (development), use localhost:2025
        // If on production (deployed), use the same host as the frontend OR a custom backend URL.
        // Since we are splitting hosting (Frontend on Netlify, Backend on Render), we need to specify the backend URL manually for production.
        
        let host;
        if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
            host = 'localhost:2025';
        } else {
            // REPLACE THIS with your Render.com backend URL after deploying!
            // Example: 'my-minecraft-server.onrender.com'
            // For now, it defaults to current host which won't work on Netlify without a proxy.
            // We will use a placeholder that the user can update easily.
            host = 'minecraft-clone-server.onrender.com'; // TODO: UPDATE THIS URL
        }
        
        console.log(`Connecting to multiplayer server at ${protocol}//${host}`);
