...
const get = (id) => {
    // Always try to get the element fresh (don't cache null values)
    // This allows retries if elements aren't ready yet
    const el = document.getElementById(id);
    if (el) {
        cache[id] = el;
        return el;
    }
    // Return cached value if element not found (might be null from previous attempt)
