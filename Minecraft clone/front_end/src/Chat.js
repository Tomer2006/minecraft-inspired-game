...
export class Chat {
    ...
    sendMessage() {
        ...
        // Send message via multiplayer if connected, otherwise display locally
        if (this.multiplayer && this.multiplayer.isConnected) {
            ...
            // Singleplayer mode: display message locally
        ...
        switch (cmd) {
            case '/fly':
                if (this.player && this.player.physics) {
                    this.player.physics.flyMode = !this.player.physics.flyMode;
                    const status = this.player.physics.flyMode ? 'enabled' : 'disabled';
                    this.addSystemMessage(`Fly mode ${status}.`);
                } else {
                    this.addSystemMessage('Error: Player physics not available.');
                }
                break;
            default:
                this.addSystemMessage(`Unknown command: ${cmd}. Available commands: /fly`);
                break;
        }
    }
