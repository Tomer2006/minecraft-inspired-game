...
export class PlayerControls {
  ...
  setupCustomControls() {
    ...
    const onMouseMove = (event) => {
      ...
      // If inventory is open, don't process mouse movement
  ...
  setupInputs() {
    const onKeyDown = (event) => {
      ...
      // Inventory Toggle - Game continues running, just disable mouse look
    ...
    const onKeyUp = (event) => {
      // Always clear movement states when keys are released, even if chat is open
      // This ensures states are cleared if keys were held when chat opened
      switch (event.code) {
        case this.player.keybinds.forward:
        case 'ArrowUp':
      ...
      // If chat is open, don't process any other controls
      if (window.chat && window.chat.isOpen) {
        return;
      }
    };
